...
for i in range(0, len(agent.memory.frames) + 1):
    ...
    state = [
        agent.memory.frames[i - 3],
        agent.memory.frames[i - 2],
        agent.memory.frames[i - 1],
        agent.memory.frames[i],
    ]
    for ind in range(4):
        state[ind] = [plt.imshow(state[ind], animated=True)]
    ani = animation.ArtistAnimation(
        fig, state, interval=750, blit=True, repeat_delay=250
    )
    ...
    plt.text(
        ...
        + str(i)
        + "    Reward: "
        + str(agent.memory.rewards[i])
        + "\nAction: "
        + str(agent.memory.actions[i])
        + "    Done: "
        + str(agent.memory.done_flags[i])
        + "\n",
        fontsize=14,
        fontweight="bold",
